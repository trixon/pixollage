...
@ActionID(
        category = "Collage",
        id = "se.trixon.pixollage.actions.ClearAction"
)
@ActionRegistration(
        displayName = "#CTL_ClearAction"
)
@ActionReferences({
    @ActionReference(path = "Menu/Collage", position = 200),
    @ActionReference(path = "Shortcuts", name = "D-DELETE")
})
@Messages("CTL_ClearAction=Clear")
public final class ClearAction extends BaseCollageAction {

    public ClearAction(Collage context) {
        mContext = context;
    }
